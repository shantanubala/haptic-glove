...
\subsubsection{Computing $P\left(X(t)/L_{ij}(t)\right)$}
...
where, $P\left(L_i(t)\right)$ can be considered as a proportionality constant and the probability of occurance of any $X(t)$ is considered to be equal. Thus,

\begin{equation}
P\left(X(t)/L_{ij}(t)\right) \propto P\left(L_{ij}(t)/X(t)\right)
\end{equation}

\subsubsection{Determining $w_{it}(t)$ given test point $X(t)$ and examples $L_{ij}(t)$}
 In [REF], the problem of determining $w_{ij}(t)$ is posed as a entropy maximization problem,

 \begin{eqnarray*}
 \mathrm{Maximize} &  \left( -\sum\limits_{i=1}^{N} \sum\limits_{j=1}^{k} w_{ij}(t) \ln\left( w_{ij}(t) \right)   \right)\\
 \mathrm{Subject to} & \sum\limits_{i=1}^{N} \sum\limits_{j=1}^{k} w_{ij}(t) =  1 \\
 & \sum\limits_{i=1}^{N} \sum\limits_{j=1}^{k} w_{ij}(t) L_{ij}(t) = X(t) \\
 \end{eqnarray*}

 The above problem is a convex optimization problem with the local maxima being the same as the global maxima. Appendix XXXX shows the solution to be problem using Newton's method.
...
\noindent Entropy Maximization under Linear Constraints Problem:
...
& x_i^* =  \frac{e^{-\beta l_i}}{\sum\limits_{i=1}^{N} e^{-\beta l_i}}
\end{align}
...
\noindent From Equation \ref{Eqn20} we identify the Lagrange Multipliers to be the $\beta$s and the solution to the required $x_i$s can then be found through the  
